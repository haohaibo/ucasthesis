...
矩阵乘的计算复杂度为$O(N^3)$，访存复杂度为$O(N^2)$，这种计算密集型的特点非常适合用GPU来加速\citepns{nath2010improved}。然而由于GPU独有的并行结构需要高度调优的kernel来达到性能峰值\citepns{kurzak2012autotuning}。当前GPU架构仍在持续演进，GPU应用的领域越来越广泛，人们对应用程序性能的极致追求推动者GPU程序调优工作的进行\citepns{cui2010auto}。
